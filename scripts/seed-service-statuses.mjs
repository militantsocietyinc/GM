...
const CACHE_TTL = 1800; // 30 minutes (matches handler)
...
function normalizeToProtoStatus(raw) {
  ...
  if (val === 'none' || val === 'operational' || val.includes('all systems operational')) return 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL';
  if (val === 'minor' || val === 'degraded_performance' || val.includes('degraded')) return 'SERVICE_OPERATIONAL_STATUS_DEGRADED';
  if (val === 'partial_outage') return 'SERVICE_OPERATIONAL_STATUS_PARTIAL_OUTAGE';
  if (val === 'major' || val === 'major_outage' || val === 'critical' || val.includes('outage')) return 'SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE';
...
async function checkServiceStatus(service) {
  const now = Date.now();
  const base = { id: service.id, name: service.name, url: service.statusPage };
  const unknown = (desc) => ({ ...base, status: 'SERVICE_OPERATIONAL_STATUS_UNSPECIFIED', description: desc, checkedAt: now, latencyMs: 0 });
  ...
  try {
    ...
    const response = await fetch(service.statusPage, { headers, signal: AbortSignal.timeout(10_000) });
    const latencyMs = Date.now() - start;

    if (!response.ok) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_UNSPECIFIED', description: `HTTP ${response.status}`, checkedAt: now, latencyMs };

    if (service.customParser === 'gcp') {
      const data = await response.json();
      const active = Array.isArray(data) ? data.filter((i) => i.end === undefined || new Date(i.end) > new Date()) : [];
      if (active.length === 0) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: 'All services operational', checkedAt: now, latencyMs };
      const hasHigh = active.some((i) => i.severity === 'high');
      return { ...base, status: hasHigh ? 'SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE' : 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: `${active.length} active incident(s)`, checkedAt: now, latencyMs };
    }

    if (service.customParser === 'aws') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: 'Status page reachable', checkedAt: now, latencyMs };
    ...
    if (service.customParser === 'rss') {
      ...
      return { ...base, status: hasIncident ? 'SERVICE_OPERATIONAL_STATUS_DEGRADED' : 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: hasIncident ? 'Recent incidents reported' : 'No recent incidents', checkedAt: now, latencyMs };
    }

    if (service.customParser === 'instatus') {
      const data = await response.json();
      const pageStatus = data.page?.status;
      if (pageStatus === 'UP') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: 'All systems operational', checkedAt: now, latencyMs };
      if (pageStatus === 'HASISSUES') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: 'Some issues reported', checkedAt: now, latencyMs };
    ...
    if (service.customParser === 'statusio') {
      ...
      if (code === 100) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: overall.status || 'All systems operational', checkedAt: now, latencyMs };
      if (code >= 300 && code < 500) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: overall.status || 'Degraded performance', checkedAt: now, latencyMs };
      if (code >= 500) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE', description: overall.status || 'Service disruption', checkedAt: now, latencyMs };
      return unknown(overall?.status || 'Unknown status');
    }

    if (service.customParser === 'slack') {
      const data = await response.json();
      if (data.status === 'ok') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: 'All systems operational', checkedAt: now, latencyMs };
    ...
    if (service.customParser === 'stripe') {
      const data = await response.json();
      if (data.largestatus === 'up') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: data.message || 'All systems operational', checkedAt: now, latencyMs };
      if (data.largestatus === 'degraded') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: data.message || 'Degraded performance', checkedAt: now, latencyMs };
      if (data.largestatus === 'down') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE', description: data.message || 'Service disruption', checkedAt: now, latencyMs };
    ...
    if (service.customParser === 'incidentio') {
      ...
      if (text.startsWith('<!') || text.startsWith('<html')) {
        if (/All Systems Operational|fully operational|no issues/i.test(text)) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: 'All systems operational', checkedAt: now, latencyMs };
        if (/degraded|partial outage|experiencing issues/i.test(text)) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: 'Some issues reported', checkedAt: now, latencyMs };
      ...
      try {
        ...
        if (indicator === 'none' || description.toLowerCase().includes('operational')) return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: description || 'All systems operational', checkedAt: now, latencyMs };
        if (indicator === 'minor' || indicator === 'maintenance') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: description || 'Minor issues', checkedAt: now, latencyMs };
        if (indicator === 'major' || indicator === 'critical') return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE', description: description || 'Major outage', checkedAt: now, latencyMs };
        return { ...base, status: 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL', description: description || 'Status OK', checkedAt: now, latencyMs };
      } catch { return unknown('Invalid response'); }
    }
    ...
    if (data.status?.indicator !== undefined) return { ...base, status: normalizeToProtoStatus(data.status.indicator), description: data.status.description || '', checkedAt: now, latencyMs };
    if (data.status?.status) return { ...base, status: data.status.status === 'ok' ? 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL' : 'SERVICE_OPERATIONAL_STATUS_DEGRADED', description: data.status.description || '', checkedAt: now, latencyMs };
    if (data.page && data.status) return { ...base, status: normalizeToProtoStatus(data.status.indicator || data.status.description), description: data.status.description || 'Status available', checkedAt: now, latencyMs };
...
runSeed('infra', 'service-statuses', CANONICAL_KEY, fetchServiceStatuses, {
  validateFn: validate,
  ttlSeconds: CACHE_TTL,
  sourceVersion: 'statuspage-v1',
}).catch((err) => {
