 ...
 * Discovers buying intent signals for a company from public sources:
 * - Job postings (via public APIs)
 * - News mentions
...
/**
 * Fetch news signals from Hacker News
 */
...
/**
 * Fetch GitHub activity signals (recent repo creation, stars spike)
 */
...
/**
 * Fetch job posting signals from public APIs
 */
async function fetchJobSignals(companyName) {
  try {
    // Use HN "Who is Hiring" threads as a proxy
    const res = await fetch(
    ...
    return [{
      type: 'hiring_surge',
      title: `${companyName} posting hiring activity (${hiringComments.length} mentions in HN hiring threads)`,
