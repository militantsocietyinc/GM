/**
 * World Clock Panel — 30 financial city clocks with market open/close status.
 * Draggable reorder, gear icon city picker, home city auto-detection.
...
export class WorldClockPanel extends Panel {
  ...
  private renderClocks(): void {
    ...
    for (const city of sorted) {
      ...
      html += `<div class="${rowCls.join(' ')}" data-city-id="${city.id}">` +
        `<div class="wc-drag-handle" title="Drag to reorder">⋮</div>` +
        `<div class="wc-info">` +
          `<div class="wc-name">${city.city}${isHome ? '<span class="wc-home-tag">⌂</span>' : ''}</div>` +
          `<div class="wc-detail"><span class="wc-exchange">${city.label}</span>${statusHtml}</div>` +
        `</div>` +
        `<div class="wc-clock">` +
